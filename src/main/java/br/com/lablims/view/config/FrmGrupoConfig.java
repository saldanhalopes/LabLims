 ...
 * @author rafael
 */
public class FrmGrupoConfig extends javax.swing.JFrame {

    /**
     * Creates new form FrmConfigGrupo
     */
    public FrmGrupoConfig() {
        initComponents();
    ...
    private void txtPesquisarKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtPesquisarKeyReleased
        TableSorter.Sorter(tblGrupos, txtPesquisar);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FrmGrupoConfig.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                new FrmGrupoConfig().setVisible(true);
            }
        });
    }
        ...
        try {
            ...
            if (dialogResult == 0) {
                ...
                try {
                    ...
                    if (senha.getSenha()) {
                        setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                        for (RegraAcesso regra : regraDAO.getListAcesso(em, (Integer) tblGrupos.getValueAt(tblGrupos.getSelectedRow(), 0))) {
                            regraDAO.remover(em, RegraAcesso.class, regra.getId());
                        }
                        grupDAO.remover(em, Grupo.class, (Integer) tblGrupos.getValueAt(tblGrupos.getSelectedRow(), 0));
    ...
    private void editarGrupo() {
        ...
        try {
            GrupoDAO grupDAO = new GrupoDAO();
            FrmGrupoDados frm = new FrmGrupoDados(null, true, grupDAO.findById(em, Grupo.class,
                    (Integer) tblGrupos.getValueAt(tblGrupos.getSelectedRow(), 0)));
            frm.setVisible(true);
            readGrupo();
        } catch (Exception ex) {
        ...
        try {
            for (Grupo grupo : grupDAO.readGruposAuditoria()) {
                model.addRow(new Object[]{
                    grupo.getId(),
                    grupo.getVersion(),
                    grupo.getMOD() == 0 ? "Criado" : (grupo.getMOD() == 1 ? "Modificado" : "Removido"),
                    grupo.getGrupoNome(),
                    grupo.getNome_MOD() == 0 ? "Não Alterado" : "Alterado",
                    grupo.getGrupoTipo(),
                    grupo.getTipo_MOD() == 0 ? "Não Alterado" : "Alterado",
