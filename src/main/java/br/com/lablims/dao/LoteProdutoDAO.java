 ...
 * @author rafael
 */
public class LoteProdutoDAO extends GenenicoDAO<LoteProduto> {

    public Boolean checkLoteProdutoIsExits(String lote) throws EntityNotFoundException, NoResultException {
        EntityManager em = ConnectionFactory.em();
        try {
            return em.createNamedQuery("LoteProduto.findLote", LoteProduto.class)
    ...
    public LoteProduto findLoteProdutoByLote(String lote) throws NoResultException {
        EntityManager em = ConnectionFactory.em();
        try {
            return em.createNamedQuery("LoteProduto.findLote", LoteProduto.class)
                    .setParameter("lote", lote)
    ...
    public LoteProduto findLoteProdutoPlanoAnalise(Long lote_id) throws NoResultException {
        EntityManager em = ConnectionFactory.em();
        try {
            return em.createNamedQuery("LoteProduto.findLoteProdutoPlanoAnalise", LoteProduto.class)
    ...
    public List<LoteProduto> findListLoteProduto(Integer maxResults,
            Map<String, String> conditional) throws Exception {
        EntityManager em = ConnectionFactory.em();
        try {
            CriteriaBuilder cb = em.getCriteriaBuilder();
            CriteriaQuery cq = cb.createQuery();
            Root<LoteProduto> root = cq.from(LoteProduto.class);
            Join<LoteProduto, LoteProdutoInfo> joinInfo = root.join("loteInfo", JoinType.INNER);
            if (conditional != null) {
    ...
    public List<Object[]> findLoteStatus(Integer maxResults) throws Exception {
        EntityManager em = ConnectionFactory.em();
        try {
            String sql = "SELECT CAST (tb_lote_produto.id AS INTEGER), tb_lote_produto.lote, "
                    + "tb_lote_produto_info.data_necessidade, tb_lote_produto_info.prev_liberacao, "
                    + "tb_lote_produto_info.obs_cq, tb_lote_produto_info.data_status, "
                    + "tb_lote_produto_info.status, tb_material.cod_material, tb_material.material, "
                    + "(Select COUNT(tb_lote_produto_status.id) FROM tb_lote_produto_status "
                    + "WHERE tb_lote_produto_status.lote_id = tb_lote_produto.id) as count_analise, "
                    + "(Select COUNT(tb_lote_produto_status.id) FROM tb_lote_produto_status "
                    + "WHERE tb_lote_produto_status.lote_id = tb_lote_produto.id "
                    + "AND tb_lote_produto_status.status_id = 4) as count_analise_finalizada, "
                    + "CAST((Select tb_plano_analise.setor_id "
                    + "FROM tb_lote_produto_status "
                    + "INNER JOIN tb_plano_analise "
                    + "ON tb_lote_produto_status.plano_analise_id = tb_plano_analise.id "
                    + "WHERE tb_lote_produto_status.lote_id = tb_lote_produto.id "
                    + "AND tb_plano_analise.setor_id = '11' "
                    + "GROUP BY tb_plano_analise.setor_id) AS INTEGER) as micro, "
                    + "(SELECT MAX(tb_lote_produto_status.previsao_data_hora) "
                    + "FROM tb_lote_produto_status "
                    + "WHERE tb_lote_produto_status.lote_id = tb_lote_produto.id) as previsaoLab, "
                    + "(SELECT DISTINCT(MAX(tb_solucao_reagente.repouso)) "
                    + "FROM tb_solucao_reagente "
                    + "WHERE tb_solucao_reagente.metodologia_id "
                    + "IN (tb_lote_produto.metodologia_id, "
                    + "tb_lote_produto.metodologia2_id, "
                    + "tb_lote_produto.metodologia3_id) "
                    + "GROUP BY tb_solucao_reagente.metodologia_id) as tempoRepouso, "
                    + "tb_lote_produto_info.data_doc "
                    + "FROM tb_lote_produto "
                    + "INNER JOIN tb_lote_produto_info "
                    + "ON tb_lote_produto.id = tb_lote_produto_info.id "
                    + "LEFT JOIN tb_material "
                    + "ON tb_lote_produto.material_id = tb_material.id "
                    + "WHERE tb_lote_produto_info.status <> 'Liberado' "
                    + "ORDER BY tb_lote_produto.id DESC "
                    + "limit :limit";
    ...
    public List<Object[]> findLoteStatus(Integer maxResults, Long pa_id) throws Exception {
        ...
        try {
            String sql = "SELECT CAST (tb_lote_produto.id AS INTEGER), tb_lote_produto.lote, "
                    ...
                    + "WHERE tb_solucao_reagente.metodologia_id IN (tb_lote_produto.metodologia_id, "
                    + "tb_lote_produto.metodologia2_id, tb_lote_produto.metodologia3_id) "
                    + "GROUP BY tb_solucao_reagente.metodologia_id) as tempoRepouso, "
                    + "tb_lote_produto_info.data_doc, "
                    + "(Select tb_analise_status.analise_status FROM tb_lote_produto_status "
                    + "INNER JOIN tb_analise_status ON tb_lote_produto_status.status_id = tb_analise_status.id "
                    + "WHERE tb_lote_produto_status.lote_id = tb_lote_produto.id "
                    + "AND tb_lote_produto_status.plano_analise_id = :pa_id) as status_analise "
                    + "FROM tb_lote_produto "
                    + "INNER JOIN tb_lote_produto_info "
                    + "ON tb_lote_produto.id = tb_lote_produto_info.id "
                    + "LEFT JOIN tb_material "
                    + "ON tb_lote_produto.material_id = tb_material.id "
                    + "WHERE tb_lote_produto_info.status <> 'Liberado' "
                    + "AND tb_lote_produto.id IN "
                    + "(SELECT tb_lote_produto_status.lote_id "
                    + "FROM tb_lote_produto_status "
                    + "WHERE tb_lote_produto_status.plano_analise_id = :pa_id "
                    + "AND tb_lote_produto_status.status_id <> 4) "
                    + "ORDER BY tb_lote_produto.id DESC "
                    + "limit :limit";
            Query query = em.createNativeQuery(sql);
            query.setParameter("limit", maxResults);
    ...
    public List<LoteProduto> readAuditoria(Integer maxResults) {
        Connection conn = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;
        List<LoteProduto> lotes = new ArrayList<>();
        try {
            stmt = conn.prepareStatement("SELECT tba_lote_produto_auditoria.*, "
                    ...
                    + "FROM tba_lote_produto_auditoria  "
                    + "INNER JOIN tbc_revinfo_auditoria "
                    + "ON tba_lote_produto_auditoria.REV = tbc_revinfo_auditoria.id "
                    + "ORDER BY tba_lote_produto_auditoria.REV DESC "
                    + "limit ?");
